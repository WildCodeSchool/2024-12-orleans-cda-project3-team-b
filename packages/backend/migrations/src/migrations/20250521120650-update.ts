...
export async function up(db: Kysely<DB>): Promise<void> {
  ...
  await db.transaction().execute(async (trx) => {
    ...
    await sql`
    ALTER TABLE artists_hired ADD skills_id INT NULL;
    `.execute(trx);
    ...
    //albums
    await sql`
    ALTER TABLE albums ADD notoreity INT NULL;
    `.execute(trx);
...
`.execute(trx);
    ...
    //artists_hired_skills
    await sql`
    ALTER TABLE artists_hired_skills change grade score  INT NOT NULL;
`.execute(trx);
    // artists_skills
    ...
    //albums
    await sql`
    ALTER TABLE albums DROP notoreity;
    `.execute(trx);
    ...
    await sql`
    ALTER TABLE artists_hired DROP COLUMN skills_id;
    `.execute(trx);
